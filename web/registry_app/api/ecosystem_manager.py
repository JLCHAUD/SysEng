"""Gestion des espaces de travail (écosystèmes) du Registry Populator N2.

Un écosystème N2 = un dossier config/ contenant :
  registre.json, acteurs.json, hierarchy.json, tables.json,
  file_types.yaml, schema_relations.json, functions.json, templates.json

L'app maintient une liste d'écosystèmes connus dans .registry_ecosystems.json
...
_REGISTRY_FILE = _PROJECT_ROOT / ".registry_ecosystems.json"
...
def _is_valid(path: Path) -> bool:
    return path.is_dir() and (path / "registre.json").exists()
...
def list_ecosystems():
    ...
    for eco in known:
        ...
        if _is_valid(p):
            try:
                with open(p / "registre.json", encoding="utf-8") as f:
                    reg = json.load(f)
                count = len(reg.get("fichiers", []))
            except Exception:
                pass
...
def new_ecosystem(body: NewEcosystemRequest):
    ...
    return EcosystemInfo(name=body.name, path=str(path), active=True, valid=True, post_count=0)
...
def open_ecosystem(body: OpenEcosystemRequest):
    ...
    if not _is_valid(path):
        raise HTTPException(422, f"Dossier invalide : registre.json manquant dans '{path}'")
...
def current_ecosystem():
    ...
    if _is_valid(path):
        try:
            with open(path / "registre.json", encoding="utf-8") as f:
                reg = json.load(f)
            count = len(reg.get("fichiers", []))
        except Exception:
            pass
    return EcosystemInfo(name=name, path=str(path), active=True, valid=_is_valid(path), post_count=count)
...
def delete_ecosystem(eco_name: str):
    ...
    # Retire de la liste (ne supprime PAS les fichiers sur disque)
