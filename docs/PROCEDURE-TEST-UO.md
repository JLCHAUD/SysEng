...
```
PUSH $liv -> uo.L09U1-TEST01-CLIM.livrables_v2
```

*(La variable `$liv` est déjà définie plus haut dans le Manifeste généré — c'est elle qui contient les livrables. On pousse vers une nouvelle clé `livrables_v2` pour éviter tout conflit avec une éventuelle clé existante.)*
...
- `python -m src status` liste une nouvelle clé `uo.L09U1-TEST01-CLIM.livrables_v2`
  avec ses lignes.
- Zéro erreur.
...
Puis vérifie les colonnes présentes dans le store avec ce snippet Python :

```python
python -c "
import json, pathlib
store_path = pathlib.Path('output/store.json')
data = json.loads(store_path.read_text(encoding='utf-8'))
rows = data.get('uo.L09U1-TEST01-CLIM.activites', [])
if rows:
    print('Colonnes :', list(rows[0].keys()))
    print('Nb lignes :', len(rows))
else:
    print('Cle absente ou vide')
"
```

**Attendu :**
- Zéro erreur.
- La liste des colonnes inclut `commentaire` en plus des colonnes d'origine.
...
- `PUSH` **reste à 9** : le PUSH s'exécute quand même, mais pousse une liste
  vide (le tableau `tbl_oil` est introuvable → GET_TABLE renvoie `[]`).
- La clé `uo.L09U1-TEST01-CLIM.oil` reste dans le store mais contient
  **0 ligne** (`[]`).
...
Puis vérifie le contenu du store :
...
- Le store contient maintenant **deux jeux de clés** :
  - `uo.L09U1-TEST01-CLIM.*` — l'ancienne identité (données périmées, non mises à jour)
  - `uo.L09U1-TEST01-CLIM-V2.*` — la nouvelle identité (données fraîches de cette sync)
- Un cockpit qui consommerait les deux voit des doublons.
